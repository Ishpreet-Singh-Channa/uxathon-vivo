...
function normalizeTeam(rawTeam?: Team | Team[] | null): Team | null {
    ...
    return {
        ...team,
        team_members: Array.isArray(team.team_members) ? team.team_members : team.team_members ? [team.team_members] : [],
    };
}
...
export default function MyTeamPage() {
    ...
    const members: TeamMember[] = Array.isArray(team?.team_members) ? team.team_members : [];

    if (!hasUserId) {
        return (
            <main className="relative min-h-screen bg-[#181818] text-white flex items-center justify-center">
                <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-[#5b5b5b]">Loading user identity…</p>
            </main>
        );
    }
    ...
    if (hasUserId && loading) {
        return (
            <main className="relative min-h-screen bg-[#181818] text-white flex items-center justify-center">
                <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-[#5b5b5b]">Syncing team assignment…</p>
            </main>
        );
    }
        ...
        return (
            <main className="relative min-h-screen bg-[#181818] text-white flex items-center justify-center px-5">
                <div className="max-w-xl text-center">
                    <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-[#ff6a6a]">GraphQL Error</p>
                    <p className="mt-2 break-words font-mono text-[11px] text-[#929292]">{error.message}</p>
                </div>
            </main>
        );
    }
                    ...
                    {new Date(team.created_at).toLocaleDateString("en-US", {
                      ...
                      year: "numeric",
                                ...
                                <div className="text-right">
                                    <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#5b5b5b]">TEAM POINTS</p>
                                    <p className="mt-2 font-sans text-[32px] uppercase tracking-[0.04em] text-[#DEF767]">{team.points ?? 0}</p>
                                </div>
                            </div>

                            {team.room_id && team.persona_id && (
                                <div className="mt-5 border-t border-[#2e2e2e] pt-4">
                                    <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[#ff6a6a]">Persona Assignment</p>
                                    <div className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-2">
                                        <p className="font-sans text-xl uppercase tracking-[0.04em] text-white">{team.persona_name || team.name}</p>

                                        {team.persona_hex && <span className="h-3 w-3 rounded-full border border-white/20" style={{ backgroundColor: team.persona_hex }} />}
                                    </div>

                                    {team.domain_name && <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.14em] text-[#929292]">Domain: {team.domain_name}</p>}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="border border-[#2e2e2e] bg-[#171717]">
                            ...
                            <div className="divide-y divide-[#2e2e2e]">
                                {members.length === 0 ? (
                                    <div className="px-6 py-5 text-center font-mono text-[10px] uppercase tracking-[0.14em] text-[#5b5b5b]">No members found for this team.</div>
                                ) : (
                                    members.map((member) => {
                                            ...
                                            <div key={member.id} className="flex items-center justify-between px-6 py-5">
                                                <div className="flex items-center gap-4">
                                                    {member.user?.profile_picture ? (
                                                        ...
                                                        <div className="grid h-12 w-12 place-items-center border border-[#2e2e2e] bg-[#181818] font-mono text-[14px] uppercase tracking-[0.08em] text-[#ff6a6a]">{getInitials(member.user?.name)}</div>
                                                    )}
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <p className="font-sans text-[18px] uppercase tracking-[0.04em] text-white">{member.user?.name || "Unknown User"}</p>
                                                            {isLeader && <Crown size={14} className="text-[#DEF767]" aria-label="Team Leader" />}
                                                        </div>
                                                        <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.14em] text-[#5b5b5b]">{isLeader ? "LEADER" : member.member_type || "MEMBER"}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        );
